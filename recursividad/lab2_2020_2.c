...
void readDatafromTerminal(int, int ,int**);
int recursiveExplore(int ncols, int nrows , 
    int x, int y , int direction  , int ** terrain,int);
...
int main(){
  ...
  int direction;
  printf("Ingresar direccion 0-arriba 1-derecha 2-abajo 3-izquiera: ");
  scanf("%d",&direction);
...
void readDatafromTerminal(int ncols, int nrows, int ** terrain){
  printf("Ingresar las condiciones del terrenos:\n ");
...
int recursiveExplore(int ncols, int nrows , int x, int y ,
    int direction  , int ** terrain,int firstIteration){
